...
//connection to int API
   /*const imageAPI = (url = 'https://mbgsp-portal-int.apac.bg.corpintra.net/tvapi/api/Image/') => {
    ...
    const imageAPI = (url ='https://localhost:44313/api/Image/?floor='+floor) => {
        return {
            fetchAll: (val) => axios.get( val == undefined ? fApi+floor: fApi+val ),
            create: newRecord => axios.post(url, newRecord),
            update: (id, updatedRecord) => axios.put(url + id, updatedRecord),
      ...
      <FormControl fullWidth>
        ...
        >
          ...
          {/* <SelectFloor/> */}
           ...
           <Grid container spacing={0.5} sx={{ backgroundColor: "whitesmoke" , objectFit:"cover",width:"100%" }}>
      {imageList.map((image) => (
       <Grid className='gridimages' item xs={1.5} sm={1.5} md={1.5} key={image.id} >
        {
        imageCard(image)
         }
        </Grid>
